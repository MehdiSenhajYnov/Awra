...
awra_split_view_update_for_width (AwraSplitView *self, int width)
{
  ...
  if (mode == AWRA_LAYOUT_MODE_MEDIUM)
    ...
    gtk_paned_set_position (self->paned, 280);
}

static void
awra_split_view_size_allocate (GtkWidget *widget, int width, int height, int baseline)
{
  awra_split_view_update_for_width (AWRA_SPLIT_VIEW (widget), width);
  GTK_WIDGET_CLASS (awra_split_view_parent_class)->size_allocate (widget, width, height, baseline);
}
...
awra_split_view_class_init (AwraSplitViewClass *klass)
{
  ...
  widget_class->size_allocate = awra_split_view_size_allocate;
...
void awra_split_view_set_sidebar (AwraSplitView *self, GtkWidget *sidebar) {
  ...
  if (sidebar != NULL) gtk_widget_set_visible (sidebar, self->show_sidebar && self->layout_mode != AWRA_LAYOUT_MODE_COMPACT);
...
void awra_split_view_set_show_sidebar (AwraSplitView *self, gboolean show_sidebar) {
  ...
  self->show_sidebar = show_sidebar; awra_split_view_update_for_width (self, gtk_widget_get_width (GTK_WIDGET (self)));
...
void awra_split_view_set_compact_width (AwraSplitView *self, guint width) {
  ...
  self->compact_width = width; awra_split_view_update_for_width (self, gtk_widget_get_width (GTK_WIDGET (self))); g_object_notify_by_pspec (G_OBJECT (self), properties[PROP_COMPACT_WIDTH]);
}
...
void awra_split_view_set_expanded_width (AwraSplitView *self, guint width) {
  ...
  self->expanded_width = width; awra_split_view_update_for_width (self, gtk_widget_get_width (GTK_WIDGET (self))); g_object_notify_by_pspec (G_OBJECT (self), properties[PROP_EXPANDED_WIDTH]);
}
