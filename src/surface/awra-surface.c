                       ...
                       GtkSnapshot *snapshot)
{
  ...
  if (self->elevation > 0)
    gtk_snapshot_append_outset_shadow (snapshot,
                                       ...
                                       &resolved.shadow,
                                       0.0,
                                       (float) MIN (self->elevation, 8U),
                                       0.0,
                                       12.0 + (float) self->elevation * 2.0);

  gtk_snapshot_push_rounded_clip (snapshot, &outline);
  gtk_snapshot_append_color (snapshot, &resolved.fill, &bounds);

  if (awra_material_get_kind (self->material) != AWRA_MATERIAL_KIND_SOLID) {
    GskColorStop stops[2] = {
      { 0.0f, resolved.highlight },
      { 1.0f, (GdkRGBA) { resolved.highlight.red,
                           resolved.highlight.green,
                           resolved.highlight.blue,
                           0.0 } },
    };
    graphene_point_t start = GRAPHENE_POINT_INIT (0, 0);
    graphene_point_t end = GRAPHENE_POINT_INIT (0, bounds.size.height * 0.55f);
    ...
    gtk_snapshot_append_linear_gradient (snapshot,
                                         ...
                                         stops,
                                         G_N_ELEMENTS (stops));
    gtk_snapshot_push_opacity (snapshot, 0.045);
  ...
  for (guint i = 0; i < G_N_ELEMENTS (widths); i++) {
    widths[i] = (float) resolved.border_width;
...
awra_surface_init (AwraSurface *self)
{
  ...
  self->material = awra_material_new_solid ();
  self->radius = -1.0;
...
{
  return g_object_new (AWRA_TYPE_SURFACE, "role", role, NULL);
}
